<?php
    ...
    require_once(plugin_dir_path(PLUGIN_FILE_URL) . "includes/services/categories_service.php");
    require_once(plugin_dir_path(PLUGIN_FILE_URL) . 'includes/models/wp_categories_model.php');
    ...
    class CategoriesController extends WP_REST_Controller {
        ...
        public function __construct() {
            $this->model=new WPCategoriesModel();
            $this->service=new CategoriesService($this->model);
        }
        ...
        public function read_by_id($request) {
            ...
            try {
                ...
                $Categorie_data=(object)$this->model->read_by_id($id);
                $response = new WP_REST_Response($Categorie_data, 200);
                $this->service->after_create($response,$request);
            } catch (Exception $erro) {
    ...
    $results_controller = new CategoriesController();
